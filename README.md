...
> **Note:** This module is aligned with [azurerm 5.0.1](https://registry.terraform.io/providers/hashicorp/azurerm/5.0.1/docs/resources/linux_virtual_machine) and supports all features of the `azurerm_linux_virtual_machine` resource including disk encryption, ephemeral OS disks, confidential VM settings, and hibernation capabilities.

## Usage

### Module Block

In your ESLZ or deployment configuration, reference this module with appropriate variables:
...
See [ESLZ/SRV-Linux-complete.tfvars](ESLZ/SRV-Linux-complete.tfvars) for comprehensive examples including disk encryption, confidential VM settings, and other advanced features.
...
| os_managed_disk_id                                     | Azure managed disk ID                    | No       | n/a                 |
| disable_password_authentication                        | true,false                               | No       | true                |
...
| public_ip_address_id           | Azure ID             | No       | n/a           |
| gateway_load_balancer_frontend_ip_configuration_id | Azure ID | No | n/a       |
| auxiliary_mode                 | AcceleratedConnections,Floating,MaxConnections,None | No | n/a |
| auxiliary_sku                  | A8,A4,A1,A2,None     | No       | n/a           |
| tags                           | map of strings       | No       | n/a           |
...
| disk_encryption_set_id                | Azure disk encryption set ID      | No       | null          |
| secure_vm_disk_encryption_set_id      | Azure disk encryption set ID      | No       | null          |
| security_encryption_type              | VMGuestStateOnly,DiskWithVMGuestState | No | null          |
| diff_disk_settings.option             | Local                             | No       | null          |
| diff_disk_settings.placement          | CacheDisk,ResourceDisk,NvmeDisk   | No       | CacheDisk     |
...
| disk_encryption_set_id            | Azure ID                | No       | null            |
| network_access_policy             | AllowAll,AllowPrivate,DenyAll | No  | null            |
| disk_access_id                    | Azure ID                | No       | null            |
| encryption_settings.disk_encryption_key.secret_url      | URI | No | null      |
| encryption_settings.disk_encryption_key.source_vault_id | Azure ID | No | null |
| encryption_settings.key_encryption_key.key_url          | URI | No | null      |
| encryption_settings.key_encryption_key.source_vault_id  | Azure ID | No | null |
| tags                              | map of strings          | No       | n/a             |
...
| Parameter Name      | Possible Value | Required | Default Value |
| ------------------- | -------------- | -------- | ------------- |
| ultra_ssd_enabled   | true,false     | No       | false         |
| hibernation_enabled | true,false     | No       | false         |
...
| source_application_security_group_ids      | list of Azure IDs | No | n/a      |
| destination_application_security_group_ids | list of Azure IDs | No | n/a      |
| tags                         | map of strings | No       | n/a           |
...
| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| <a name="input_custom_data"></a> [custom\_data](#input\_custom\_data) | Base64 encoded file representing custom data script for the VM. Also accepts the keywords "install-ca-certs" or "cloud-init-default" (both resolve to a G3/non-G3-specific public blob based on var.env), or a URL to fetch the script/cloud-init content from directly. | `any` | `null` | no |
