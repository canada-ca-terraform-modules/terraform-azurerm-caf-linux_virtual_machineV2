locals {
  ...
  postfix_3                     = substr(try(var.linux_VM.postfix, ""), 0, 3)
  userDefinedString_7           = substr(var.userDefinedString, 0, 7 - length(local.postfix_3))
  vm-name                       = replace("${local.env_4}${local.serverType_3}-${local.userDefinedString_7}${local.postfix_3}", local.linux_virtual_machine_regex, "")
}