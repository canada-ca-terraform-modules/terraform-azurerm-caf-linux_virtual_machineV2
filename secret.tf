...
data "azurerm_key_vault" "key_vault" {
  count = try(var.linux_VM.password_overwrite, false) || try(var.linux_VM.disable_password_authentication, true) ? 0 : 1
  name = local.kv_name
...
resource "random_password" "vm-admin-password" {
  count            = try(data.azurerm_key_vault.key_vault[0].enable_rbac_authorization, false) && !try(var.linux_VM.password_overwrite, false) && !try(var.linux_VM.disable_password_authentication, true) ? 1 : 0
  length           = 16
  special          = true
  override_special = "!#$%&*"
  min_lower        = 1
